...
void readReg(uint8_t reg, uint8_t *buf, size_t len)
{
  ...
  Wire.endTransmission(false); //restarts message after transmitting register address
  Wire.write(0x32);
  Wire.requestFrom(reg, len);  //requests len bytes from reg
  for(int i = 0; i < len; i++) {
  	buf[i] = Wire.read();
  }
